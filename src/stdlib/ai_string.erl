...
-spec atom_suffix(Name :: string() | binary() | atom(),
                  Suffix :: string() | binary() | atom(),
                  Exist :: boolean()) -> atom().
atom_suffix(Name,Suffix,Exist)->
  Name0 = to_string(Name),
  Suffix0 = to_string(Suffix),
  StrName  = <<Name0/binary,Suffix0/binary>>,
  to_atom(StrName,Exist).

-spec atom_prefix(Name :: string() | binary() | atom(),
                  Suffix :: string() | binary() | atom(),
                  Exist :: boolean()) -> atom().
atom_prefix(Name,Prefix,Exist)->
  Name0 = to_string(Name),
  Prefix0 = to_string(Prefix),
  StrName  = <<Prefix0/binary,Name0/binary>>,
  to_atom(StrName,Exist).

to_atom(Name,false)-> erlang:binary_to_atom(Name,utf8);
to_atom(Name,true)-> erlang:binary_to_existing_atom(Name,utf8).
